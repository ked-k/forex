...
@section('content')
			<div class="page-content">
				...
				<!--end breadcrumb-->
				<h6 class="mb-0 text-uppercase">Receive Payment</h6>
				<hr/>
                    <div class="row">
                        <div>
                            <div class="card">
                                <div class="card-header pt-0">
                                    <form method="POST" action="{{ url('forex/payments/add/r') }}" name="myForm"  class="needs-validation"  onsubmit="return validateForm()">
                                       @csrf
                                        <div class="row mb-2 mt-3">
                                            <input type="hidden" class="form-control" name="reff_number"  readonly value="{{ rand(8960,765555).time() }}">
                                            <div class="col-sm-4">
                                                <div class="text-sm">
                                                <label>Clearing Customer</label>
                                                <select class="single-select form-select" name="from_id" id="to_id" required >
                                                ...
                                                <input type="hidden" readonly  class="form-control"  name="from_acct" id="to_acct" required>
                                            ...
                                            $('#to_id').change(function() {
                                               ...
                                               // document.getElementById('total_amount').value =0;
                                                ...
                                                if (acct2) {
                                                    $.ajax({
                                                        type: "GET",
                                                        url: "{{ url('forex/payments/getcust') }}?sup_id=" + acct2,
                                                        success: function(response) {
...
<script>
 ...
 if (q1 < 1) {
   ...
   swal('Error', 'You cant not exceed the required amount!', 'warning');
    return false;
  }
  else if (bal > 0) {
   swal('Error', 'You cant not exceed the required amount!', 'warning');
